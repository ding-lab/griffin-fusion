...
top_10_names <- names(sort(table(primary_df$fusion), decreasing=TRUE)[1:10])
top_10_count <- as.vector(sort(table(primary_df$fusion), decreasing=TRUE)[1:10])
top_10_rate <- vector()
for(name in top_10_names){
  val <- subset(primary_df, fusion==name)$n_discordant
  rate <- mean(val >= 3 , na.rm=T)
  top_10_rate <- c(top_10_rate, rate)
}
...
plot_df <- data.frame(fusion=top_10_names, n_samples=top_10_count, validation_rate=format(top_10_rate,digits=1))
plot_df$fusion <- factor(plot_df$fusion, levels = rev(top_10_names))
ggplot(plot_df, aes(x=fusion, weight=n_samples, label=validation_rate)) + geom_bar() + coord_flip() + labs(x="Fusion name", y="Number of samples", title="Top 10 Recurrent Fusions") + theme_bw(base_size=20) + geom_text(aes(x = fusion, y = n_samples), hjust=1, color="white", size=5)
...
plot_df <- data.frame(n_fusions = n_fusions)
med <- median(plot_df$n_fusions)
ggplot(plot_df, aes(x=n_fusions)) + geom_histogram() + labs(x="Number of Fusions", y="Number of Samples", title="Number of Fusions per Sample") + theme_bw(base_size=20) + geom_vline(xintercept=med, lty=2, color="white", size=1) + geom_text(label=paste0("Median=",med), x=med, y=50, angle=90, vjust=-1, color="white", size=5)
...
ggplot(plot_df, aes(x=Freq)) + geom_histogram() + theme_bw(base_size=20) + labs(x="Number of Samples", y="Number of Fusions", title="Number of Samples per Fusion")
ggplot(subset(plot_df, Freq>1), aes(x=Freq)) + geom_histogram() + theme_bw(base_size=20) + labs(x="Number of Samples", y="Number of Fusions", title="Number of Samples per Fusion")

#FGFR3 WHSC1 expression with fusion annotation
srr_with_fgfr3_fusion <- unique(subset(primary_df, (geneB %in% c("FGFR3") & geneA=="IGH@"))$srr) #| (geneB %in% c("FGFR3") & geneA=="IGH@"))$srr)
srr_with_whsc1_fusion <- unique(subset(primary_df, (geneB %in% c("WHSC1") & geneA=="IGH@"))$srr) #| (geneB %in% c("WHSC1") & geneA=="IGH@"))$srr)
