...
fgfr3_log10tpm <- subset(expr, gene %in% c("FGFR3") & srr %in% samples$V2)[,c("srr","log10tpm")]
whsc1_log10tpm <- subset(expr, gene %in% c("WHSC1") & srr %in% samples$V2)[,c("srr","log10tpm")]
...
ggplot(plot_df, aes(x=bp_igh/1000, y=bp_whsc1/1000, color=as.factor(round(ex_fgfr3)))) + geom_point(size=3) + theme_bw(base_size=20) + labs(x="IGH Breakpoint (kb) (chromosome 14)", y="WHSC1 Breakpoint (kb) (chromosome 4)", color="Expression (rounded)\nlog10(TPM)", title="IGH--WHSC1 Fusion Genomic Breakpoints") + theme(axis.text.x = element_text(angle = 90, hjust = 1))

ggplot(subset(plot_df, bp_igh > 106300000), aes(x=bp_igh/1000, y=bp_whsc1/1000, color=as.factor(round(ex_fgfr3)))) + geom_point(size=3) + theme_bw(base_size=20) + labs(x="IGH Breakpoint (kb) (chromosome 14)", y="WHSC1 Breakpoint (kb) (chromosome 4)", color="Expression (rounded)\nlog10(TPM)", title="IGH--WHSC1 Fusion Genomic Breakpoints") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
