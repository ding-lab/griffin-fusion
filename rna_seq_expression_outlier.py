...
try:
  sys.argv[4]
    ...
    outlier_levels=int(sys.argv[4])
  	...
  	sys.exit("Outlier level parameter must be an integer > 0.")
...
for gene in gene_list:
    ...
    for i in range(outlier_levels):
    	output_list.append(int(expr > overexpression_outlier_level_list[i]))
    for i in range(outlier_levels):
    	output_list.append(int(expr < underexpression_outlier_level_list[i]))
