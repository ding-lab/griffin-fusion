...
if (TRUE) {
  ...
  ggplot(data = drug_df, aes(x = fct_reorder(gene, n_samples), y = n_samples, label = evidence)) +
    ...
    ggsave(str_c(paper_supp, "drug_targets.pdf"), width = 7.25, height = 7.25/1.618, useDingbats = FALSE)
...
# ==============================================================================
# Survival association with fusion events -- including R-ISS and cytogenetics
# ==============================================================================

if (TRUE) {

  Rseqfish_clinical_info <- seqfish_clinical_info %>%
    mutate(RISS_Stage = case_when((ISS_Stage == 1 & HRD == 0 & LDH <= 280) ~ 1,
                                  (ISS_Stage == 3 & (HRD == 1 | LDH > 280)) ~ 3,
                                  TRUE ~ 2)) %>%
    mutate(RISS_Stage = factor(RISS_Stage, levels = c(1,2,3), ordered = TRUE))
  ...
  fusions_ge10 <- Rseqfish_clinical_info %>%
    filter(!is.na(RISS_Stage), !is.na(EFS_censor), !is.na(Age),
           ...
           !is.na(updated_seqfish_t_IGH_MYC), !is.na(updated_seqfish_t_IGK_MYC),
           !is.na(updated_seqfish_t_IGL_MYC)) %>%
    left_join(fusions_primary, by = "mmrf") %>%
    filter(!is.na(fusion)) %>%
    select(fusion) %>%
    group_by(fusion) %>%
  ...
  for (this_fusion in fusions_ge10) {
    ...
    if (EFS_tibble %>% pull(fusion) %>% unique() %>% length() > 1) {
      baseline_coxph_model_EFS <- coxph(Surv(EFS, EFS_censor == 0) ~ RISS_Stage + Age + SeqWGS_Cp_Hyperdiploid_Call + updated_seqfish_t_IGH_CCND1 + updated_seqfish_t_IGH_WHSC1 + updated_seqfish_t_IGH_MAF + updated_seqfish_t_IGH_CCND3 + updated_seqfish_t_IGH_MAFB + updated_seqfish_t_IGH_MYC + updated_seqfish_t_IGK_MYC + updated_seqfish_t_IGL_MYC, data = EFS_tibble)
      coxph_model_EFS <- coxph(Surv(EFS, EFS_censor == 0) ~ RISS_Stage + Age + fusion + SeqWGS_Cp_Hyperdiploid_Call + updated_seqfish_t_IGH_CCND1 + updated_seqfish_t_IGH_WHSC1 + updated_seqfish_t_IGH_MAF + updated_seqfish_t_IGH_CCND3 + updated_seqfish_t_IGH_MAFB + updated_seqfish_t_IGH_MYC + updated_seqfish_t_IGK_MYC + updated_seqfish_t_IGL_MYC, data = EFS_tibble)
      p_value_EFS <- summary(coxph_model_EFS)$coefficients[str_c("fusion", this_fusion), 5]
      p_value_model_comparison <- anova(baseline_coxph_model_EFS, coxph_model_EFS)["P(>|Chi|)"][2,1]

      if (p_value_EFS < 0.05 & p_value_model_comparison < 0.05) {
        Rcoxph_model_EFS_list[[this_fusion]] <- coxph_model_EFS
      }
    }
  }
  ...
  for (this_gene in genes_ge10) {
    ...
    if (EFS_tibble %>% pull(gene) %>% unique() %>% length() > 1) {
      baseline_coxph_model_EFS <- coxph(Surv(EFS, EFS_censor == 0) ~ RISS_Stage + Age + SeqWGS_Cp_Hyperdiploid_Call + updated_seqfish_t_IGH_CCND1 + updated_seqfish_t_IGH_WHSC1 + updated_seqfish_t_IGH_MAF + updated_seqfish_t_IGH_CCND3 + updated_seqfish_t_IGH_MAFB + updated_seqfish_t_IGH_MYC + updated_seqfish_t_IGK_MYC + updated_seqfish_t_IGL_MYC, data = EFS_tibble)
      coxph_model_EFS <- coxph(Surv(EFS, EFS_censor == 0) ~ RISS_Stage + Age + gene + SeqWGS_Cp_Hyperdiploid_Call + updated_seqfish_t_IGH_CCND1 + updated_seqfish_t_IGH_WHSC1 + updated_seqfish_t_IGH_MAF + updated_seqfish_t_IGH_CCND3 + updated_seqfish_t_IGH_MAFB + updated_seqfish_t_IGH_MYC + updated_seqfish_t_IGK_MYC + updated_seqfish_t_IGL_MYC, data = EFS_tibble)
      p_value_EFS <- summary(coxph_model_EFS)$coefficients[str_c("gene", this_gene), 5]
      p_value_model_comparison <- anova(baseline_coxph_model_EFS, coxph_model_EFS)["P(>|Chi|)"][2,1]

      if (p_value_EFS < 0.05 & p_value_model_comparison < 0.05) {
        Rcoxph_model_EFS_list[[this_gene]] <- coxph_model_EFS
      }
    }
  }
...
if (TRUE) {
  ...
  ggplot(structure_tbl, aes(xmin = start,
    ...
    theme(panel.background = element_blank(),
          ...
          axis.text.y = element_blank(),
          axis.ticks = element_blank()) +
    ggsave(str_c(paper_supp, "NTRK1.structures.pdf"),
...
if (TRUE) {
  pancan_fusions %>%
    ...
    separate(Fusion, sep = "--", into = c("geneA", "geneB")) %>%
...
if (TRUE) {
  ...
  apobec_outlier_gt <- max(apobec_outlier_true, 0.5)
  ...
  n_high_apobec <- apobec_background_high %>% filter(gene == "All samples", APOBEC >= apobec_outlier_true) %>% nrow()
  n_with_signature_score <- apobec_background_high %>% filter(gene == "All samples") %>% nrow()
  ...
  ggplot(apobec_background_high,
    ...
    theme(panel.background = element_blank(),
          ...
          legend.direction = "vertical",
          axis.title = element_text(size = 12)) +
    ggsave(str_c(paper_supp, "apobec.pdf"), width = 2.5, height = 1, useDingbats = FALSE)
}

# ==============================================================================
# APOBEC signature association with fusion events expanded
# ==============================================================================
...
if (TRUE) {
  ...
  outlier_apobec <- rbind(fusions_primary %>%
    ...
    summarize(apobec_median = median(APOBEC),
              count = n()) %>%
    filter(apobec_median > apobec_outlier_true)
  ...
  ggplot(apobec_background_high,
    ...
    theme(panel.background = element_blank(),
          ...
          legend.direction = "vertical",
          axis.title = element_text(size = 12)) +
    ggsave(str_c(paper_supp, "apobec.expanded.pdf"), width = 7.25, height = 2, useDingbats = FALSE)
}
...
multiple_kinase_overlap <- tcga_kinase_overlap %>% ungroup() %>% filter(count2 > 1) %>%
  separate(geneB_mmrf_count, into = c("gene", "count"), sep = c(" ")) %>%
  mutate(count = str_remove(count, c("\\("))) %>%
  mutate(count = str_remove(count, c("\\)"))) %>%
...
# Correlation of APOBEC score and total fusions
mutsig %>% select(mmrf, srr, visit, APOBEC) %>% left_join(seqfish_clinical_info %>% select(mmrf,total_fusions)) %>% summarize(cor(APOBEC, total_fusions))

# APOBEC high with MAF or MAFB translocation
apobec_background_high %>%
  filter(gene == "All samples", APOBEC > apobec_outlier_true) %>%
  left_join(seqfish_clinical_info %>%
              select(mmrf,
                     ...
                     updated_seqfish_t_IGL_MAFB), by = "mmrf") %>%
  rowwise() %>%
  mutate(any_MAF = sum(updated_seqfish_t_IGH_MAF,
                       updated_seqfish_t_IGK_MAF,
                       updated_seqfish_t_IGL_MAF,
                       updated_seqfish_t_IGH_MAFB,
                       updated_seqfish_t_IGK_MAFB,
