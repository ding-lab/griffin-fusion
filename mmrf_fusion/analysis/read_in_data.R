...
# Integration of multiple time points
# From: https://satijalab.org/seurat/v3.0/immune_alignment.html
# integrate 27522 -- only do one time
if (FALSE) {
  combined_anchors <- FindIntegrationAnchors(object.list = list(seurat_object_27522_1, seurat_object_27522_4), dims = 1:20)
  combined <- IntegrateData(anchorset = combined_anchors, dims = 1:20)
  DefaultAssay(combined) <- "integrated"
  combined <- ScaleData(combined, verbose = FALSE)
  combined <- RunPCA(combined, npcs = 30, verbose = FALSE)
  combined <- RunUMAP(combined, reduction = "pca", dims = 1:20)
  write_rds(x = combined, path = "data/scRNA.seurat_object.27522_combined.rds", compress = "none")
  rm(combined_anchors, combined)
} else {
  seurat_object_27522_integrated <- read_rds("data/scRNA.seurat_object.27522_combined.rds")
}

# integrate 56203 -- only do one time
if (FALSE) {
  combined_anchors <- FindIntegrationAnchors(object.list = list(seurat_object_56203_1, seurat_object_56203_2), dims = 1:20)
  combined <- IntegrateData(anchorset = combined_anchors, dims = 1:20)
  DefaultAssay(combined) <- "integrated"
  combined <- ScaleData(combined, verbose = FALSE)
  combined <- RunPCA(combined, npcs = 30, verbose = FALSE)
  combined <- RunUMAP(combined, reduction = "pca", dims = 1:20)
  write_rds(x = combined, path = "data/scRNA.seurat_object.56203_combined.rds", compress = "none")
  rm(combined_anchors, combined)
} else {
  seurat_object_56203_integrated <- read_rds("data/scRNA.seurat_object.56203_combined.rds")
}

# integrate 81012 -- only do one time
if (FALSE) {
  combined_anchors <- FindIntegrationAnchors(object.list = list(seurat_object_81012_1, seurat_object_81012_2), dims = 1:20)
  combined <- IntegrateData(anchorset = combined_anchors, dims = 1:20)
  DefaultAssay(combined) <- "integrated"
  combined <- ScaleData(combined, verbose = FALSE)
  combined <- RunPCA(combined, npcs = 30, verbose = FALSE)
  combined <- RunUMAP(combined, reduction = "pca", dims = 1:20)
  write_rds(x = combined, path = "data/scRNA.seurat_object.81012_combined.rds", compress = "none")
  rm(combined_anchors, combined)
} else {
  seurat_object_81012_integrated <- read_rds("data/scRNA.seurat_object.81012_combined.rds")
}