...
if (TRUE) {
  ...
  mmrf_with_primary_mutation_calls <- mutation_calls %>%
    separate(Tumor_Sample_Barcode, into = c("MMRF", "NUM", "VISIT"), sep = "_") %>%
  ...
  pdf(str_c(paper_main, "survival.WHSC1_fusion_EFS.forest.pdf"), width = 3.75, height = 3.75)
  print(ggforest(plot_survival_list[["WHSC1_fusion_EFS"]], data = EFS_tibble, noDigits = 3))
  dev.off()
  ...
  pdf(str_c(paper_supp, "survival.WHSC1_fusion_expression_mutation_EFS.forest.pdf"), width = 3.5, height = 2)
  print(ggforest(plot_survival_list[["with_fusion_mutation_expression_EFS"]], data = EFS_tibble %>% filter(fusion == "IGH--WHSC1"), noDigits = 3))
  dev.off()
...
################################################################################
# Added for FGFR3 RNA DNA VAF comparison
################################################################################

fgfr3_vaf <- read_tsv("data/FGFR3_VAF.txt")
ggplot(data = fgfr3_vaf,
       aes(x = dna_tumor_VAF, y = rna_tumor_VAF)) +
  geom_abline(linetype = 2, color = "grey50") +
  geom_abline(slope = 2, linetype = 2, color = "grey50") +
  geom_point(aes(color = mmrf), shape = 16, show.legend = FALSE) +
  geom_label_repel(aes(label = mmrf, color = mmrf), show.legend = FALSE, size = 2) +
  coord_equal() +
  theme_bw() +
  labs(color = "MMRF Sample Name",
       x = "DNA Mutation VAF",
       y = "RNA Mutation VAF") +
  ...
  theme(panel.background = element_blank(),
        ...
        legend.title = element_text(size = 12),
        legend.position = "bottom") +
  ggsave(str_c(paper_supp, "fgfr3_dna_rna_vaf.pdf"),
       width = 2.25, height = 2.25, useDingbats = FALSE)
