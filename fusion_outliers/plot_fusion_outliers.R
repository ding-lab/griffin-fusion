...
p <- p + geom_violin(aes(fill=NULL),color="black")
p <- p + geom_point(aes(x=fusion_status_j, alpha=alpha_level))
p <- p + geom_label_repel(data=subset(plot_df, fusion_status=="Fusion"), aes(x=fusion_status_j, y=expression_level, label=gene_partners), label.size=NA, color=c('green','black')[as.numeric(subset(plot_df, fusion_status=="Fusion")$cnv=="No Data")+1], box.padding=0.35, point.padding=0.5, segment.color="grey50")
